...
export default function seriesPage() {
    ...
    async function getEpisodes() {
        try {
            const episodesData = await axios.get(`http://${IP}:3000/episodes?seriesId=${seriesPage}`);
    ...
    async function getSeasonAmount() {
        try{
            const series = await axios.get(`http://${IP}:3000/series`);
    ...
    return (
        <SafeAreaView>
           ...
           {filterEpisodes.length > 0 ? (
                filterEpisodes.map((ep, index)=><Text key={index}>{ep.title}</Text>)
